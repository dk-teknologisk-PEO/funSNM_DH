function T_soil = get_soil_temperature(city, t_start,t_end, api_key)%lat, lon, number_of_weather_stations, combined_data)
...
% period from t_start to t_end.
    ...
    for date=dates
        ...
        for j=1:length(values)
            if ~strcmp(data.features(j).properties.timeResolution,'hour')
                continue
            end
        ...
        T_soil_temporary = rmmissing(timetable(time,values));

        % add to main timetable
        if ~exist("T_soil")
            T_soil = T_soil_temporary;
        else
            T_soil = [T_soil;T_soil_temporary;];
        end
    end
    % remove rows with no timestamp
    T_soil(isnat(T_soil.time),:)=[];
    % sort the table chronologically
    T_soil = sortrows(T_soil,"time", "ascend");
    % remove duplicates
    T_soil = unique(T_soil);
