...
%
%   Creates two figures:
%   1. Offset estimates: initial vs final vs true, with uncertainty bars
%   2. U-value estimates: initial vs final vs true, with uncertainty bars
    ...
    bar(x + bar_width/2, final_offset, bar_width, 'FaceColor', [0.2 0.5 0.8], ...
        'DisplayName', 'Final');
    % Uncertainty bars
    errorbar(x - bar_width/2, init_offset, init_std_offset, 'k.', 'LineWidth', 1, ...
        'HandleVisibility', 'off');
    errorbar(x + bar_width/2, final_offset, final_std_offset, 'k.', 'LineWidth', 1, ...
        'HandleVisibility', 'off');
    % True values
    ...
    xlabel('House');
    ylabel('Offset [°C]');
    title(sprintf('CSAC %d — Offset: Initial vs Final Estimate', csac_id));
    ...
    bar(x + bar_width/2, final_U, bar_width, 'FaceColor', [0.8 0.5 0.2], ...
        'DisplayName', 'Final');
    errorbar(x - bar_width/2, init_U, init_std_U, 'k.', 'LineWidth', 1, ...
        'HandleVisibility', 'off');
    errorbar(x + bar_width/2, final_U, final_std_U, 'k.', 'LineWidth', 1, ...
        'HandleVisibility', 'off');
    ...
    title(sprintf('CSAC %d — U-value: Initial vs Final Estimate', csac_id));
    legend('Location', 'best');
    grid on;
    hold off;
