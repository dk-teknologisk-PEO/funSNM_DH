...
%   2) reference_direct           - reference sensor at EVERY main-pipe/CSAC
%                                   junction from day one. Reference
%                                   temperature is taken NOISE-FREE from the
%                                   data file (J_Main_<id>_s) and assigned
%                                   directly to cs.T_inlet_from_main.
%                                   T_inlet_fitted is completely bypassed; no
%                                   propagation along the main pipe, no
%                                   inverse-variance fusion.
%   3) reference_after_convergence - same as (2) but the reference-sensor
%                                   override only activates after
%                                   CONVERGENCE_DAYS of elapsed simulation
%                                   time (default: 30 days). Before that the
%                                   CSAC runs independently like in (1).
% get_supply_temp.m still handles the main-junction -> service-pipe step
% inside process_csac_timestep in all scenarios.
...
% Pre-scan topologies to determine the maximum number of CSACs across all
% networks (network 4 has more CSACs than networks 1-3, so we cannot use a
% hard-coded value when padding per-CSAC result rows). Read the topology
% JSON files directly to avoid the slow meter-data import.
max_csacs = 0;
...
CONVERGENCE_DAYS = 30;  % delay before reference sensor kicks in (scenario 3)
scenario_defs(1).name = 'no_sensor';
scenario_defs(1).use_reference = false;
scenario_defs(1).activation_days = 0;
scenario_defs(2).name = 'reference_direct';
scenario_defs(2).use_reference = true;
scenario_defs(2).activation_days = 0;
scenario_defs(3).name = 'reference_after_convergence';
scenario_defs(3).use_reference = true;
scenario_defs(3).activation_days = CONVERGENCE_DAYS;
...
for net_idx = 1:num_networks
    ...
    for s = 1:num_scenarios
        ...
        activation_days = scenario_defs(s).activation_days;
        activation_time = timestamps(1) + days(activation_days);
        ...
        for t = 1:length(timestamps)
            ...
            if use_reference && time >= activation_time
                for c = 1:num_csacs
                    if ~ref_data{c}.valid
                        continue
                    end
        ...
        % One figure per CSAC, one line per house, full simulation span.
        ts_folder = fullfile('results', 'sensor_test_direct', ...
            sprintf('net%d', network_id), scenario_defs(s).name);
        ...
        for c = 1:num_csacs
            ...
            % --- Subplot 1: meter offset error vs time ---
            ...
            title(sprintf('Net %d, CSAC %d, scenario: %s — meter offset error', ...
                network_id, csac_ids(c), strrep(scenario_defs(s).name, '_', '\_')));
            legend(leg, 'Location', 'eastoutside');
            hold off;

            % --- Subplot 2: service-pipe U-value error vs time ---
...
for k = 1:max_csacs
    hdr = [hdr, sprintf(' | %8s', sprintf('CSAC%d', k-1))]; %#ok<AGROW>
end
fprintf('%s\n', hdr);
fprintf('%s\n', repmat('-', 1, numel(hdr)));
for s = 1:num_scenarios
    ...
    row = sprintf('%-28s', a.name);
    for k = 1:max_csacs
        row = [row, sprintf(' | %8.3f', avg_per_csac(k))]; %#ok<AGROW>
    end
    fprintf('%s\n', row);
end
fprintf('========================================\n');
