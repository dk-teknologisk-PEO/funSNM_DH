...
R_base = config.project.initialization.measurement_noise^2;
Q_base = diag([(config.project.initialization.process_noise_offset)^2, (config.project.initialization.process_noise_U)^2]);
P_base = diag([(config.project.initialization.state_uncertainty_offset)^2, (config.project.initialization.state_uncertainty_U)^2]);
num_particles = config.project.initialization.num_particles;
...
flow_gate_threshold = config.project.cutoff.flow_gate_threshold;
...
for network = networks
    ...
    % u_history = nan([size(topology.houses,1),length(timestamps)]);
    % offset_history = nan([size(topology.houses,1),length(timestamps)]);
    ...
    for csac=csac_ids
        ...
        % initialize a Kalman filter for each house on the csac and store
        ...
        for t = 1:length(timestamps)
            ...
            T_junction_ukf_C = calculate_main_pipe_temp(current_data, current_T_soil_C, U_csac, flow_cutoff, ukf_states);
            ...
            for i = 1:num_houses_csac
                ...
                if house_data.flow_kg_h<config.project.cutoff.flow_cutoff
                    continue
                end

                T_main_ukf_local = house_data.T_main_ukf_C;
                T_main_pf_local = house_data.T_main_pf_C;
                flow_sufficient = house_data.flow_kg_h >= flow_gate_threshold;
                delta_T_ukf_sufficient = (T_main_ukf_local - current_T_soil_C) >= delta_T_gate_threshold;
                delta_T_pf_sufficient = (T_main_ukf_local - current_T_soil_C) >= delta_T_gate_threshold;
                if flow_sufficient
                    if delta_T_ukf_sufficient
                    % update UKF and particle filter
                        [ukf_states{i}, diagnostics_ukf] = update_ukf_house(ukf_states{i}, house_data, current_T_soil_C);
                    else
                        if t > 1
                        previous_ukf_state.x = logger_ukf.state_estimates(:, i, t-1);
                        previous_ukf_state.P = diag(logger_ukf.covariance_posterior(:, i, t-1));
                        end
                    end

                    if delta_T_pf_sufficient
                        ...
                        if t > 1
                            previous_pf_state.x = logger_ukf.state_estimates(:, i, t-1);
                            previous_pf_state.P = diag(logger_ukf.covariance_posterior(:, i, t-1));
                        end
                    end
                end
